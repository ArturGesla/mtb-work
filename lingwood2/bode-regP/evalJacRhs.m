...
%     %xmom
    ...
    g(iif)=u(iif)^2-u(iig)^2+(fn-fs)/(zn-zs)*hp-(dfn-dfs)/(zn-zs);
...
%     %ymom
    ...
    g(iig)=2*u(iif)*u(iig)+(gn-gs)/(zn-zs)*hp-(dgn-dgs)/(zn-zs);
%     g(iig)=g(iig)+2*u(iif);
...
g(iif)=(u(iif)+u(iifzm))/2; g(iig)=(u(iig)+u(iigzm))/2-k; g(iih)=u(iih)-u(iih-4); g(iip)=u(iip)-u(iip-4);
