...
%z mom
%conti
% i=length(zw)-2; iip=(i-1)*4+1; iih=iip+3;
% g(iih)=u(iip+4); %pressure pert at wall = 0 %this generates not full rank jac
% ii(iii)=iih; jj(iii)=iip+4; vv(iii)=1;iii=iii+1;
for i=2:length(zw)-2
...
%     %xmom
    ...
    g(iif)=u(iif)*(1i*bbar*U(iig)-1i*omega)+hp*(Fn-Fs)/(zn-zs);
...
%     ii(iii)=iif; jj(iii)=iif; vv(iii)=(1i*bbar*U(iig)-1i*omega); iii=iii+1;
    ...
    ii1(iii1)=iif; jj1(iii1)=iip; vv1(iii1)=1i;iii1=iii1+1;
...
%     g(iif)=g(iif)+1/R*u(iif)*U(iif)+1/R*U(iih)*(fn-fs)/(zn-zs)-2/R*U(iig)*u(iig)-2/R*u(iig)+u(iif)*bbar^2/R;
    g(iif)=g(iif)+1/R*u(iif)*U(iif)+1/R*Hp*(fn-fs)/(zn-zs)-2/R*U(iig)*u(iig)+u(iif)*bbar^2/R;
    
    ii(iii)=iif; jj(iii)=iif; vv(iii)=1/R*U(iif);iii=iii+1;
    
    ii(iii)=iif; jj(iii)=iif; vv(iii)=1/R*Hp*((zpp-zn))/(zpp-zp)/(zn-zs);iii=iii+1;
    ii(iii)=iif; jj(iii)=iifzp; vv(iii)=1/R*Hp*((zn-zp))/(zpp-zp)/(zn-zs);iii=iii+1;
    ii(iii)=iif; jj(iii)=iifzm; vv(iii)=-1/R*Hp*((zp-zs))/(zp-zpm)/(zn-zs);iii=iii+1;
    ii(iii)=iif; jj(iii)=iif; vv(iii)=-1/R*Hp*((zs-zpm))/(zp-zpm)/(zn-zs);iii=iii+1;
...
%     %y mom
    ...
    Gn=(U(iig)*(zpp-zn)+U(iigzp)*(zn-zp))/(zpp-zp);
    Gs=(U(iigzm)*(zp-zs)+U(iig)*(zs-zpm))/(zp-zpm);
%     pp=(u(iip)+u(iipzm))/2; %al def
%     
        dgn=(u(iigzp)-u(iig))/(zpp-zp);
        dgs=(u(iig)-u(iigzm))/(zp-zpm);
    
    g(iig)=u(iig)*(1i*bbar*U(iig)-1i*omega)+hp*(Gn-Gs)/(zn-zs)+1i*bbar*pp;
...
%     ii(iii)=iig; jj(iii)=iig; vv(iii)=(1i*bbar*U(iig)-1i*omega);iii=iii+1;
    ...
    ii(iii)=iig; jj(iii)=iip; vv(iii)=bbar*1i;iii=iii+1;
...
%     g(iig)=g(iig)+2/R*u(iif)*U(iig)+1/R*U(iif)*u(iig)+1/R*U(iih)*(gn-gs)/(zn-zs)+2/R*u(iif)+1/R*bbar^2*u(iig);
    g(iig)=g(iig)+2/R*u(iif)*U(iig)+1/R*U(iif)*u(iig)+1/R*Hp*(gn-gs)/(zn-zs)+1/R*bbar^2*u(iig);
    
    ii(iii)=iig; jj(iii)=iif; vv(iii)=2/R*U(iig);iii=iii+1;
    ii(iii)=iig; jj(iii)=iig; vv(iii)=1/R*U(iif);iii=iii+1;
    
    ii(iii)=iig; jj(iii)=iig; vv(iii)=1/R*Hp*((zpp-zn))/(zpp-zp)/(zn-zs);iii=iii+1;
    ii(iii)=iig; jj(iii)=iigzp; vv(iii)=1/R*Hp*((zn-zp))/(zpp-zp)/(zn-zs);iii=iii+1;
    ii(iii)=iig; jj(iii)=iig; vv(iii)=-1/R*Hp*((zs-zpm))/(zp-zpm)/(zn-zs);iii=iii+1;
    ii(iii)=iig; jj(iii)=iigzm; vv(iii)=-1/R*Hp*((zp-zs))/(zp-zpm)/(zn-zs);iii=iii+1;
...
g(iip)=u(iip)-u(iipzp); %a bit cheating bcs p inr first cell in never ref but ok
g(iif)=(u(iif)+u(iifzp))/2; g(iig)=(u(iig)+u(iigzp))/2; g(iih)=u(iih);%(u(iih)+u(iihzp))/2;
...
g(iip)=u(iip)-u(iip-4); % again slight cheat
g(iih)=(u(iih)-u(iihzm)); %this h nev ref
ii(iii)=iih; jj(iii)=iih; vv(iii)=1;iii=iii+1;
ii(iii)=iih; jj(iii)=iihzm; vv(iii)=-1;iii=iii+1;

g(iif)=(u(iif)+u(iifzm))/2; g(iig)=(u(iig)+u(iigzm))/2;  
ii(iii)=iif; jj(iii)=iif; vv(iii)=1/2;iii=iii+1;
...
% 
% 
% g(iih-4)=u(iip); %pressure pert at wall = 0 %this generates not full rank jac
% ii(iii)=iih-4; jj(iii)=iip; vv(iii)=1;iii=iii+1;

g(iih-4)=(u(iih-4)); %h pert at inf somehow artificial bcs conti says that it is h' that is 0 not h
ii(iii)=iih-4; jj(iii)=iih-4; vv(iii)=1;iii=iii+1;
