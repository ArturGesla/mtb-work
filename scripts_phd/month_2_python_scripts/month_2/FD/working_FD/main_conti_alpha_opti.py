...
for i in range(0,1000):
    u_dot_prev=u_dot;
    [u_0,u_dot,lam_0,lam_dot]=pc_iteration(u_0,u_dot,lam_0,lam_dot,h,bc,ds);
    R=np.append(R,lam_0);
    C=np.append(C,u_0[-1]);
    print ('------------------> i',i)
    ds=ds*alpha_aim/np.arccos(u_dot@u_dot_prev/norm(u_dot)/norm(u_dot_prev))
#plot_prof(u_0,bc,fields,l);
...
#%%
save=[u_0,u_dot,lam_0,lam_dot,R,C];

for i in range(0, len(save)):
    #np.savetxt(str('./'+str(i)), np.array([save[i]]), delimiter = ',');
    np.save(str('./data/'+str(i)), np.array([save[i]]));

    
#%%
save_read=[None]*6;
for i in range(0, len(save_read)):
    #np.savetxt(str('./'+str(i)), np.array([save[i]]), delimiter = ',');
    save_read[i]=np.load(str('./data/'+str(i)+'.npy'));
    
[u_0,u_dot,lam_0,lam_dot,R,C]=save_read;
R=R[0];
C=C[0];
