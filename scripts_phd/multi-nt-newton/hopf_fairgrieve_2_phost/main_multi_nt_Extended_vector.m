...
%     
    ...
    g(ix)=u(np*neq-1-neq)-u(ix);
    g(iy)=u(np*neq-neq)-u(iy);
    
    J(ix,ix)=-1; J(ix,np*neq-1-neq)=1; 
    J(iy,iy)=-1; J(iy,np*neq-neq)=1; 
  ...
  J(ix,ix)=1; J(ix,1+neq)=-1; 
  J(iy,iy)=1; J(iy,2+neq)=-1; 
...
%
[evc,evs]=eig(A,B); evs=diag(evs); b2=abs(evs)<Inf; evs=evs(b2) 
lam=1./(1-evs) % one of lambda should be 1
exp=1./T*log(abs(lam))
%%
hold on;
...
ie=2;
plot(u(1:2:end-1)+real(evc(1:2:end,ie)),u(2:2:end-1)+real(evc(2:2:end,ie)));
plot([u(1:2:end-1),u(1:2:end-1)+real(evc(1:2:end,ie))]',[u(2:2:end-1), u(2:2:end-1)+real(evc(2:2:end,ie))]');
...
%  [t,X] = rk4(@subhopfp, 10*np,T/(np-2),mu,b,om,0,[r;0],[0;0]);
T=abs(T);
 [t,X] = rk4(@subhopfp, 1000*np/10,T/(np-2)/10,mu,b,om,0,[r;0],[0;0]);
...
rr=sqrt(X(:,1).^2+X(:,2).^2);
r=rr(end);
semilogy(t,abs(rr-r),'-x'); hold on;
%%
close all;
% rl=log(abs(rr-r)); b=abs(rl)<inf; b(1:1900)=0;
rl=log(abs(rr-r)); b=abs(rl)<inf; b(10:end)=0;
a=polyfit(t(b),rl(b),1);
plot(t,rl); hold on;
plot(t(b),rl(b)); hold on;
plot(t,a(1)*t+a(2))
% xlim([0 2*max(t(b))])
a
%% Fairgrieve part
...
[evc,evs]=eigs(A,B,neq,"smallestabs")
% lam=1./(1-evs) % one of lambda should be 1
% exp=1./T*log(abs(lam))
