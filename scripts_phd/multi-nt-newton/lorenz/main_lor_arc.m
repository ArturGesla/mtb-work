...
%
%
neq=3; r=29; b=8/3; sigma=10;
% r=15;
T=1.5586;
%
np=100; %valid points, no repeats
dt=T/(np);
g=zeros(neq*np+1,1);
J=zeros(neq*np+1);
Jlam=zeros(neq*np+1,1);
...
%     xNextNext=u(mod(ip*2-1+neq+neq-1,neq*np)+1); yNext=u(mod(ip*2+neq-1,neq*np)+1);
 ...
 uMC=[uMC,u];
%% at this point whole 0th state is known
lam=r;
% u0=u;
%assume
u_dot=u*0;
...
%
% tic
x=sparse([J,Jlam;u_dot',lam_dot])\[zeros(neq*np+1,1);1];
% toc
u_dot=x(1:end-1);
lam_dot=x(end);
norm(x)
...
r
rM=[rM;r];
uNormM=[uNormM;norm(u)];
%%
 plot3(u(1:neq:end-1),u(2:neq:end-1),u(3:neq:end-1))
plot(rM,uNormM,'x-')
%%
