...
ix=(PetscInt*)malloc(n * sizeof(PetscInt));
//vals=(PetscScalar*)malloc(n * sizeof(PetscScalar));
...
//PetscCall(VecSetValue(x,i,i,INSERT_VALUES));
//PetscCall(VecSetValues(x,n,ix,vals,INSERT_VALUES));

PetscCall(VecGetArray(x,&vals));
...
PetscCall(VecRestoreArray(x,&vals));
//free(ix);
//free(vals);
