...
for ikl=0:nt-1
    for ikr=0:nt-1
        ...
        g(3:neq:end-1)=g(3:neq:end-1)+1/2*(u(ixl)*u(ixr))*(cos((ikl+ikr)*acos(collx))+cos(abs(ikl-ikr)*acos(collx)))+1/2*(u(iyl)*u(iyr))*(cos((ikl+ikr)*acos(collx))+cos(abs(ikl-ikr)*acos(collx)));
...
for ikl=0:nt-1
    ...
    g(1:neq:end-1)=g(1:neq:end-1)+mu*(u(ixl))*(cos((ikl)*acos(collx)))-(u(iyl))*(cos((ikl)*acos(collx)));
    g(2:neq:end-1)=g(2:neq:end-1)+mu*(u(iyl))*(cos((ikl)*acos(collx)))+(u(ixl))*(cos((ikl)*acos(collx)));
    g(3:neq:end-1)=g(3:neq:end-1)-(u(izl))*(cos((ikl)*acos(collx)));


    ii=[ii,1:neq:nt*neq]; jj=[jj,ixl*ones([1,nt])];  vv=[vv; mu*(cos((ikl)*acos(collx)))];
    ii=[ii,1:neq:nt*neq]; jj=[jj,iyl*ones([1,nt])];  vv=[vv; -(cos((ikl)*acos(collx)))];

    ii=[ii,2:neq:nt*neq]; jj=[jj,iyl*ones([1,nt])];  vv=[vv; +mu*(cos((ikl)*acos(collx)))];
    ii=[ii,2:neq:nt*neq]; jj=[jj,ixl*ones([1,nt])];  vv=[vv; (cos((ikl)*acos(collx)))];

    ii=[ii,3:neq:nt*neq]; jj=[jj,izl*ones([1,nt])];  vv=[vv; -(cos((ikl)*acos(collx)))];
